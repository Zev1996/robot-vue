...
export default {
  ...
  data() {
    return {
      ...
      value_replay:false,
      value_bundle:false,
      topic:"/"+localStorage.getItem('dev_id')+"/sonar/0/realtime",
      topic_replay:"/"+localStorage.getItem('dev_id')+"/sonar/0/replay",
      option: {
        left: 'center',
        top: 'middle',
        title: {
  ...
  watch:{
    value_replay: function(){
      if(this.value_replay===true){
           ...
           data0=[];
          this.client.send("/"+localStorage.getItem('dev_id')+"/sonar/0/timemachine", "replay_open");
          this.client.subscribe(this. topic_replay);      
   
      }else if(this.value_replay===false){
         this.client.send("/"+localStorage.getItem('dev_id')+"/sonar/0/timemachine", "replay_close");
         data0=[];
