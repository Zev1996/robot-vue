...
export default {
  ...
  data() {
    return {
      ...
      value_replay:false,
      value_bundle:false,
      topic:"/1/laser/0/realtime",
      topic_replay:"/1/laser/0/replay",
      option: {
        left: 'center',
        top: 'middle',
        title: {
  ...
  watch:{
    value_replay: function(){
      if(this.value_replay===true){
           ...
           data0=[];
          this.client.send("/1/laser/0/timemachine", "replay_open");
          this.client.subscribe(this. topic_replay);      
   
      }else if(this.value_replay===false){
         this.client.send("/1/laser/0/timemachine", "replay_close");
         data0=[];
  ...
  methods: {
    ...
    open() {
     this.client.send("/1/laser/0/timemachine", "record_open");
     console.log('开始录制。。')
    },
    close() {
     this.client.send("/1/laser/0/timemachine", "record_close");
     console.log('结束录制。。')
    },
    CpuStatDonut() {
